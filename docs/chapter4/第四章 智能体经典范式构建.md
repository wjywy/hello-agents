...
本书的实战部分将主要使用 Python 语言，建议使用 Python 3.10 或更高版本。同时，我们也会提供 Node.js 版本的实现示例。首先，请确保你已经安装了相应的依赖库用于与大语言模型交互，以及用于安全地管理我们的 API 密钥的库。
...
如果你使用 Node.js，建议使用 Node.js 18 或更高版本。在你的终端中运行以下命令：
...
class HelloAgentsLLM {
     ...
     * 它用于调用任何兼容OpenAI接口的服务，并默认使用流式响应。
     */
    constructor(model = null, apiKey = null, baseUrl = null, timeout = null) {
        /**
         * 初始化客户端。优先使用传入参数，如果未提供，则从环境变量加载。
         */
        ...
        timeout = timeout || parseInt(process.env.LLM_TIMEOUT || '60', 10);
        ...
        this.client = new OpenAI({
            apiKey: apiKey,
            baseURL: baseUrl,
            timeout: timeout * 1000, // OpenAI SDK 使用毫秒
        });
    }
